const calc = (size, material, options, promocode, result) => {
	...
	const calcFunc = () => {
		const selectedSizeOption =
			sizeElement.selectedIndex !== -1
				? sizeElement.options[sizeElement.selectedIndex]
				: null
		const selectedMaterialOption =
			materialElement.selectedIndex !== -1
				? materialElement.options[materialElement.selectedIndex]
				: null
		const selectedServicesOption =
			optionsElement.selectedIndex !== -1
				? optionsElement.options[optionsElement.selectedIndex]
				: null

		const sizePriceRaw = selectedSizeOption?.dataset.sizePrice || ''
		const materialPriceRaw =
			selectedMaterialOption?.dataset.materialPrice || ''
		const servicesPriceRaw =
			selectedServicesOption?.dataset.servicesPrice || '0'
		...
		const isSizeValid =
			sizePriceRaw !== '' && !isNaN(sizePrice) && sizePrice > 0
